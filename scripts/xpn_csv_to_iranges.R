...
limmafile = 'results/limma_results.csv'
remoat.annot.file <- 'lib/Annotation_Illumina_Mouse-WG-V1.1_mm9_V1.0.0_Aug09.txt'
#'lib/Annotation_Illumina_Mouse-WG-V1_mm9_V1.0.0_Aug09.txt'   ???
...
g.ranges <- sapply(limma.annot[,"Genomic_location"], function(x){strsplit(x,',|\\s')})
...
#retrieve the ensembl ID from the Proportion_Ensembl_transcripts column
ensembl.gene.id<-limma.annot[,"Proportion_Ensembl_transcripts"]
ensembl.gene.id<-gsub('.*\\(','', ensembl.gene.id)
ensembl.gene.id<-gsub('\\)','',ensembl.gene.id)

#add it to the annotation file
limma.annot<-cbind(limma.annot, ensembl.gene.id)
...
save(rd.limma, file='results/limma_rd.RData')

rownames(g.ranges) <- rownames(limma.annot) <- nms
write.csv(cbind(g.ranges,limma.annot), file="results/limma_rd.csv", row.names=F)
